...
from geotransformer.modules.transformer.srpe_transformer import SRPETransformerLayer
        ...
        for i, block in enumerate(self.blocks):
            if block == 'self':
                ...
                if self.parallel:
                    ...
                    new_feats0, scores0 = self.layers[i](feats0, sem0_self, memory_masks=masks1)
                    new_feats1, scores1 = self.layers[i](feats1, sem1_self, memory_masks=masks0)
                    sem0 = new_feats0
                    sem1 = new_feats1
                else:    
                    ...
                    sem0, scores0 = self.layers[i](feats0, sem0_self, memory_masks=masks1)
                    sem1, scores1 = self.layers[i](feats1, sem1_self, memory_masks=masks0)
...
class SSARPEConditionalTransformer(nn.Module):
    def __init__(
        self,
        blocks,
        d_model,
        ...
        super(SSARPEConditionalTransformer, self).__init__()
        self.blocks = blocks
        layers = []
        for block in self.blocks:
            _check_block_type(block)
            if block == 'self':
                layers.append(SRPETransformerLayer(d_model, num_heads, dropout=dropout, activation_fn=activation_fn))
            elif block == 'cross':
                layers.append(TransformerLayer(d_model, num_heads, dropout=dropout, activation_fn=activation_fn))
        self.layers = nn.ModuleList(layers)
        self.return_attention_scores = return_attention_scores
    ...
    def forward(self, feats0, feats1, sem0, sem1, embeddings0, embeddings1, masks0=None, masks1=None):
        ...
        for i, block in enumerate(self.blocks):
            if block == 'self':
                # Geometric Self-Attention
                feats0, scores0 = self.layers[i](feats0, feats0, embeddings0, sem0, memory_masks=masks0)
                feats1, scores1 = self.layers[i](feats1, feats1, embeddings1, sem1, memory_masks=masks1)
