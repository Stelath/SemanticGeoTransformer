r"""Transformer with Relative Positional Embeddings & Semantic Embeddings.
...
class SRPEMultiHeadAttention(nn.Module):
    ...
    def forward(self, input_q, input_k, input_v, embed_qk1, embed_qk2, key_weights=None, key_masks=None, attention_factors=None):
        ...
        p = rearrange(self.proj_p(embed_qk1), 'b n m (h c) -> b h n m c', h=self.num_heads)
        s = rearrange(self.proj_p(embed_qk2), 'b m (h c) -> b h m c', h=self.num_heads)

        attention_scores_p = torch.einsum('bhnc,bhnmc->bhnm', q, p)
        attention_scores_s = torch.einsum('bhnc,bhmc->bhnm', q, s)
        attention_scores_e = torch.einsum('bhnc,bhmc->bhnm', q, k)
        attention_scores = (attention_scores_e + attention_scores_p + attention_scores_s) / self.d_model_per_head ** 0.5
...
class SRPEAttentionLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None):
        super(SRPEAttentionLayer, self).__init__()
        self.attention = SRPEMultiHeadAttention(d_model, num_heads, dropout=dropout)
...
class SRPETransformerLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None, activation_fn='ReLU'):
        super(SRPETransformerLayer, self).__init__()
        self.attention = SRPEAttentionLayer(d_model, num_heads, dropout=dropout)
