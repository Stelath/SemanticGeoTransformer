...
def process_sequences(sequences, sequences_dir, args):
    ...
    for sequence in tqdm(sequences, desc='Sequences'):
        ...
        for starting_index in range(0, len(bin_files) - 100, 5):
            p1_name = bin_files[starting_index]
            p2_name = bin_files[starting_index + 100]
            ...
            p2_gt_transform = np.concatenate([gt_transforms.iloc[starting_index + 50].to_numpy().reshape(3, 4), np.array([[0, 0, 0, 1]])], axis=0)
